...
export default class Sailboats extends Phaser.GameObjects.Container {
    private dtc: DTC = new DTC();
    private sailboat!: Phaser.GameObjects.Sprite;
    private isAnimating: boolean = false;
    private readonly defaultScale: number = 1.0;
    private readonly targetScale: number = 1.4;
    ...
    constructor(scene: Phaser.Scene) {
        ...
        this.createSailboat(true);
    }

    private createSailboat(isInitial:boolean): void {
        var strSailboatName: string = 'sailboat-01';
        const startY = Phaser.Math.Between(490, 500); 
        const duration = Phaser.Math.Between(40000, 80000); 

        let startX = 950;    // Start far to the right, outside the visible area
        if(isInitial) {
            startX = Phaser.Math.Between(600, 950); 
        }

        this.sailboat = this.scene.add.sprite(startX, startY, strSailboatName);
        this.sailboat.setScale(1);
        this.sailboat.setOrigin(0.5, 1.0);
        this.add(this.sailboat);
        
        this.sailboat.setInteractive();
        this.sailboat.on('pointerdown', this.handleClick, this);
        ...
        this.scene.tweens.add({
            ...
            onComplete: (tween, targets) => {
                ...
                const completedSailboat = targets[0] as Phaser.GameObjects.Sprite;
                ...
                completedSailboat.destroy(); 
                this.createSailboat(false);
            }
        });
    }
    ...
    private animateClick(): void {
        ...
        const scaleUpTween = this.scene.tweens.add({
            targets: this.sailboat,
            scaleY: this.targetScale, // Scale up to 1.2
            ...
            const scaleDownTween = this.scene.tweens.add({
                targets: this.sailboat,
                scaleY: this.defaultScale, // Scale down to 1.0 (original size)
    ...
    private showPopupText(): void {
        ...
        const popupX = this.sailboat.x;
        const popupY = this.sailboat.y + (this.sailboat.height * this.sailboat.scaleY) / 2;
